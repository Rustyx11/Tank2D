    ...
    while (!WindowShouldClose())        // Okno gry bedzie otwarte do czasu nacisniecia klawisza ESC lub przycisku zamykajacego okno
    {
        ...
        while(gamemode == GAME)
        {
            ...
            if(!is_game_loaded || map_loaded != which_map)      // osbluga rozpoczecia gry
            {
                ...
                right_pannel = LoadTexture("img/right_pannel.png");       //usprawnic
            ...
            if(!WindowShouldClose())
            {
                ...
                if(player.lives > 0)
                    DrawTextureEx(player.tank_direction, (Vector2)
                {
                    player.x, player.y
                },0, 1, RAYWHITE);  // rysowanie textury gracza
                drawEnemies(enemies);
               ...
               if(draw_win_end_window(end_window, win_sound) == true);
                PlaySound(win_sound);
